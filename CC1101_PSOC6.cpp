...
uint8_t CC1101::begin(volatile uint8_t &My_addr)
{
    ...
    spi_write_strobe(SFTX);  cyhal_system_delay_us(100);//flush the TX_fifo content
    spi_write_strobe(SFRX);  cyhal_system_delay_us(100);//flush the RX_fifo content
...
uint8_t CC1101::rx_payload_burst(uint8_t rxbuffer[], uint8_t &pktlen)
{
    ...
    else
    {
        ...
        sidle();                                                  //set to IDLE
        spi_write_strobe(SFRX);  cyhal_system_delay_us(100);            //flush RX Buffer
    ...
    else
    {
        ...
        else                                               //real data, and send acknowladge
        {
            ...
            if(debug_level > 0){                           //debug output messages
                ...
                printf(" |");
                printf("%x%x", rxbuffer[pktlen+1], rxbuffer[pktlen+2]);
                printf("|\n");

                printf("RSSI: %d, LQI: %x, CRC: %x\n",rssi_dbm, lqi, crc);
            }
...
//|==================== SPI Interface Initialization for CC1101 =========================|
void CC1101::spi_begin(void)
{

    //|--- Activating the SPI interface of PSOC62 --------|
    //|--- The Macros are defined in pins.h -------------|

   /* configure all SPI related pins */
    cy_rslt_t result = cyhal_spi_init( &hal_SPI, SPI_MOSI, SPI_MISO, SPI_SCLK,
                             SPI_CSN, NULL, 8, CYHAL_SPI_MODE_00_MSB, false);
    if (result != CY_RSLT_SUCCESS)
    {
        handle_error();
    }
    result = cyhal_spi_set_frequency(&hal_SPI, 1000000);    // CC1101 works around 1MHz clock;
    if (result != CY_RSLT_SUCCESS)
    {
        handle_error();
    }
...
void CC1101::GDO_set()
{
    cyhal_gpio_init(SPI_GDO0, CYHAL_GPIO_DIR_INPUT, CYHAL_GPIO_DRIVE_PULLDOWN, 0);
    cyhal_gpio_init(SPI_GDO2, CYHAL_GPIO_DIR_INPUT, CYHAL_GPIO_DRIVE_PULLDOWN, 0);
}
...
{
    uint32_t status;
    Cy_SCB_SPI_Write(hal_SPI.base, data);
    while (Cy_SCB_SPI_IsTxComplete(hal_SPI.base) == false) { }
    while (Cy_SCB_SPI_GetNumInRxFifo(hal_SPI.base) == 0) { } /* Wait for RX FIFO not empty */
    status = Cy_SCB_SPI_Read(hal_SPI.base);
...
void CC1101::spi_write_strobe(uint8_t spi_instr)
{
    ...
    while (SPI_SI_IS_HIGH());  // Wait until MOSI_PIN becomes LOW
    spi_putc(spi_instr);
...
void CC1101::uart_puthex_nibble(const unsigned char b)
{
    ...
    printf("%c", c);                  //uart_putc replaced to Serial.write
} /* uart_puthex_nibble */
